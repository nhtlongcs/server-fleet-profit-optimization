...
datacenter_slots = {k: v-70 for k, v in datacenter_slots.items()}
...
# apply ceiling to the prices
# pricing_strategy['price'] = pricing_strategy['price'].apply(lambda x: math.ceil(x))
# pricing_strategy['price'] = pricing_strategy['price'].apply(lambda x: int(x))
# pricing_strategy = df_pricing.set_index(['time_step', 'server_generation', 'latency_sensitivity']).to_dict()
...
with open(f'detect_ts_less_than.pkl', 'rb') as f:
    detect_ts_less_than = pickle.load(f)

with open(f'detect_ts_equal_to.pkl', 'rb') as f:
    detect_ts_equal_to = pickle.load(f)

with open(f'detect_ts_greater_than.pkl', 'rb') as f:
    detect_ts_greater_than = pickle.load(f)
...
for ts in time_steps:
    # 0 Demand Update - Pricing Strategy 

    ts_prices = get_time_step_prices(pricing_strategy, int(ts))

    # UPDATE THE SELLING PRICES ACCORDING TO PRICES AT TIMESTEP ts
    updated_selling_prices = update_selling_prices(selling_prices, ts_prices)
    if ts in detect_ts_less_than: # Z < D
        updated_selling_prices = 1.02 * updated_selling_prices
    elif ts in detect_ts_greater_than: # Z>D
        updated_selling_prices = 1.02 * updated_selling_prices
    else:
        updated_selling_prices = updated_selling_prices.copy()
    selling_prices_dict = updated_selling_prices.to_dict()
    for sg in server_types:
        for ls in latency_sensitivity:
            price_at_ts = hiring_prices[(sg, ls)]
            # new_price_at_ts = math.ceil(selling_prices_dict[ls][sg])
            new_price_at_ts = selling_prices_dict[ls][sg]
            e_at_ts = elasticity[(sg, ls)]
            demand_vars[ts, sg, ls] = get_new_demand_for_new_price(demand.get((ts, sg, ls), 0), price_at_ts, new_price_at_ts, e_at_ts)
            # demand here is a constant
    ...
    for sg in server_types:
        for ls in latency_sensitivity:
            ...
            if demand_vars[ts, sg, ls] == min_recorded[ts][sg][ls]:
                model.addConstr(min_var[ts, sg, ls] == demand_vars[ts, sg, ls], name=f"min_demand_{ts}_{sg}_{ls}")
            elif demand_vars[ts, sg, ls] > min_recorded[ts][sg][ls]:
                model.addConstr(min_var[ts, sg, ls] <= demand_vars[ts, sg, ls], name=f"min_demand_{ts}_{sg}_{ls}")
    ...
    # if ts in detect_ts_less_than:
    #     revenue_t = gp.quicksum(
    #         (min_var[ts, sg, ls] * 1.02 * selling_prices_dict[ls][sg])
    #         for sg in server_types for ls in latency_sensitivity
    #     )
    # elif ts in detect_ts_equal_to:
    #     revenue_t = gp.quicksum(
    #         (min_var[ts, sg, ls] * selling_prices_dict[ls][sg])
    ...
    #     for sg in server_types for ls in latency_sensitivity
    # )
    # revenue_t2 = gp.quicksum(
    #     Zf * price_vars[ts, sg, ls]
    #     for sg in server_types for ls in latency_sensitivity
    # )    
    # Cost at time step t
...
rich.print(f"Utilization: {total_U.getValue():0,.2f}")
rich.print(f"Profit: {total_profit.getValue():0,.2f}")
rich.print(f"- Revenue: {R.getValue():0,.2f}")
rich.print(f"- Cost: {C.getValue():0,.2f}")
    ...
    for ts in range(begin_ts + 1,168 + 1):
        ...
        for sg in server_types:
            for ls in latency_sensitivity:
                ...
                diff_recorded += Zf_recorded[ts][sg][ls] - demand_vars[ts, sg, ls]
